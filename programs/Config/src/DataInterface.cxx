    ...
    Fl_Group(x, y, width, height, label), usrpConfigStruct_(usrpConfigStruct)
{
	...
	shared_ptr<DataWindowInterface>(new DataWindowInterface(usrpConfigStruct_, 
							      x0+170, y0+5,
							      w0, h0, NULL));
    dataWindowInterface_->box(FL_ENGRAVED_BOX);
    this->add(dataWindowInterface_.get());

    addButton_ = shared_ptr<Fl_Button>(new Fl_Button(x0+10,y0+70,70,25,"Add"));
    ...
    removeButton_ = shared_ptr<Fl_Button>(new Fl_Button(x0+90,y0+70,70,25,"Remove"));
    removeButton_->box(FL_PLASTIC_DOWN_BOX);
    removeButton_->callback(DataInterface::RemoveClicked,
			    dataWindowInterface_.get());
    this->add(removeButton_.get());
    
    ippInput_ = shared_ptr<Fl_Int_Input>(new Fl_Int_Input(x0+30,y0+20,50,25,"IPP"));
    ippInput_->callback(DataInterface::Update,this);
    ippInput_->value("10");
    this->add(ippInput_.get());

    unitsChoice_ = shared_ptr<Fl_Choice>(new Fl_Choice(x0+90,y0+20,70,25,NULL));
