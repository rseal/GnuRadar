...
public class ConfigFile implements Cloneable {
	...
	public String getPriUnits() {
		return priUnits;
	}

	public void setPriUnits(String priUnits) {
		this.priUnits = priUnits;
	}

	public double getTxCarrier() {
		return txCarrier;
	}

	public void setTxCarrier(double txCarrier) {
	...
	public void setFpgaImage(String fpgaImage) {
		this.fpgaImage = fpgaImage;
	}
	...
	public static File Serialize(ConfigFile file) {
		...
		for (Channel ch : file.getChannels()) {
			Control.Channel channel = Control.Channel.newBuilder()
					...
					.setPhaseUnits(ch.getpUnits()).build();
		...
		for (Window win : file.getWindows()) {
			Control.Window window = Control.Window.newBuilder()
					.setName(win.getName()).setStart((float) win.getStart())
					.setStop((float) win.getStop()).setUnits(win.getUnits())
					.build();

			windows.add(window);
		}

		File f = File.newBuilder().addAllWindow(windows)
				.addAllChannel(channels).setVersion(file.getVersion())
				...
				.setIpp((float) file.getPri()).setIppUnits(file.getPriUnits())
				.setNumChannels(channels.size()).setNumWindows(windows.size())
				.setOrganization(file.getOrganization())
				.setRadar(file.getRadar()).setReceiver(file.getReceiver())
				.setSite(file.getSite())
				.setTxCarrier((float) file.getTxCarrier())
				.setUser(file.getUser()).build();
