...
public class FileSettingsPanel extends JPanel implements ApplicationSettings {

	private static final long serialVersionUID = 1L;

	private final static File CONFIGURATION_DIRECTORY = new File(
			"/usr/local/gnuradar/rev4");
	...
	public FileSettingsPanel() {
		...
		new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (result == JFileChooser.APPROVE_OPTION) {
					...
					fpgaImageTextField.setText(fpgaImageFile.getAbsolutePath());
				}
			}
		});
		...
		fileBaseNameButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (result == JFileChooser.APPROVE_OPTION) {
					...
					fileBaseNameTextField.setText(baseNameFile
							.getAbsolutePath());
				}
			}
		});
		...
		TitledBorder tBorder = new TitledBorder(border, "File Settings");
		this.setBorder(tBorder);
	}

	@Override
	public void getSettings(ConfigFile configuration) {
		configuration.setFpgaImage(fpgaImageTextField.getText());
		configuration.setBaseFile(fileBaseNameTextField.getText());
	}

	@Override
	public void pushSettings(ConfigFile configuration) {
		fpgaImageTextField.setText(configuration.getFpgaImage());
		fileBaseNameTextField.setText(configuration.getBaseFile());
	}
}
